...
mod tests {
  ...
  /// **Codex regression**: pre-fix `cpu_frame_bytes` used `linesize ×
  /// plane_height_for(AVFrame.height)`. On a cropped or heavily aligned
  /// stream the underlying buffer can be far larger than
  /// `AVFrame.height` (display) suggests — the formula then under-
  /// counted the allocation by `allocated_height / display_height` and
  /// let the real allocation slip past `max_probe_pending_bytes`.
  ///
  /// Build a 256-byte buffer, attach it as `buf[0]`, but set
  /// `AVFrame.height` to 1 to simulate a cropped display. The new
  ...
  /// **Codex regression**: `send_packet` must NOT consume the packet
  /// through the active decoder if the probe rescue cannot record it.
  /// Pre-fix the order was `state.inner.send_packet → cap check →
  /// abandon probe → return Ok` — by the time probe was abandoned the
  /// packet was already in FFmpeg's state but missing from
  /// `buffered_packets`, so a later runtime exhaustion would surface
  /// `unconsumed_packets` without that packet and a non-seekable
  /// caller could not rebuild the input stream.
  ///
  ...
  /// **Codex regression**: when `advance_probe` exhausts the probe
  /// (no more candidates and the active backend just failed), the
  /// `Err(AllBackendsFailed { unconsumed_packets, .. })` it returns
  /// must include the packets the decoder has already consumed from
  /// the caller's demuxer. For non-seekable inputs (live streams,
  /// pipes, network sources), losing those packets means the caller's
  /// software fallback cannot replay the initial bytes and silently
  /// drops the leading frames.
  ///
  ...
  /// **Codex regression**: pre-fix, `ProbeState.attempts` was always
  /// initialised to `Vec::new()`, even when `open` had already
  /// accumulated failures from earlier backends in probe order. A
  /// runtime exhaustion on the active backend would then return an
  /// `AllBackendsFailed` whose `attempts` log only mentioned the
  /// active backend's failure — VAAPI's earlier open failure was lost.
  ///
  /// Post-fix, `open` seeds `ProbeState.attempts` with the local
  /// `attempts` vec via `mem::take`, so a runtime exhaustion now
  /// surfaces the full failure chain in probe order.
  ///
